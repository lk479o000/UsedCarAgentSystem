<template>
  <div class="animate-fade-in">
    <UCard>
      ...
      <ULoading :loading="loading">
        <UTable :data="tableData" :columns="columns">
          ...
          <template #action="{ row }">
            <div class="flex gap-2">
              <UButton v-if="userStore.isAdmin" type="ghost" size="sm" @click="handleUpdate(row)">更新状态</UButton>
              <UButton v-if="userStore.isAdmin" type="ghost" size="sm" @click="handleNotify(row)">推送提醒</UButton>
            </div>
          </template>
        </UTable>
      </ULoading>
    ...
    <!-- 更新状态对话框 -->
    <UDialog v-model="statusDialogVisible" title="更新结算状态" width="400px">
...
const statusForm = reactive({ status: 0, settledAt: null, remark: '' })
